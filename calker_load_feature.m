...
function [feats, labels] = calker_load_feature(proj_name, exp_name, ker, video_pat)
    ...
    if (any(ismember(MEDMD.EventKit.EK10Ex.eventids, video_pat))), 
        if strcmp(ker.prms.rtype, 'NR'),
            clips = [MEDMD.EventKit.(ker.prms.eventkit).judge.(video_pat).positive];
            labels = ones(1, length(clips)));
        else
            clips = [MEDMD.EventKit.(ker.prms.eventkit).judge.(video_pat).positive, MEDMD.EventKit.(ker.prms.eventkit).judge.(video_pat).miss];
            if strcmp(ker.prms.rtype, 'RP')
                labels = [ones(1, length(clips)];
            elseif strcmp(ker.prms.rtype, 'RN')
                labels = [ones(1, length(MEDMD.EventKit.(ker.prms.eventkit).judge.(video_pat).positive)), -ones(1, length(MEDMD.EventKit.(ker.prms.eventkit).judge.(video_pat).miss)];
            else
        ...
        switch video_pat,
            ...
            case 'medtest14'
                clips = MEDMD.RefTest.MEDTEST.clips;
            otherwise
    ...
    for ii = 1:length(clips), %
        ...
        segment_path = sprintf('%s/%s/feature/%s/%s/%s/%s.mat',...
                        ker.proj_dir, proj_name, ker.prms.seg_name, ker.feat_raw, fileparts(ker.MEDMD.lookup.(clip_name)), clip_name);
