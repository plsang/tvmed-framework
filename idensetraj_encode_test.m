function idensetraj_encode_test( exp_name, pat_list, test_video_id )
    ...
    for ss = 1:length(clips),
	
		video_id = clips{ss};
		
		if ~strcmp(test_video_id, video_id), continue; end;
        ...
        if bool_run == 0, 
   		...
   		fprintf(' [%d] Extracting & Encoding for [%s], durations %f s...\n', ss, video_id, info.(video_id).duration);
